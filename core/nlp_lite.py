...
core/nlp_lite.py
----------------
Bundled English stopwords for serverless (no NLTK download at runtime).
"""

from __future__ import annotations

# NLTK English stopword list (embedded — ~180 tokens)
ENGLISH_STOPWORDS: frozenset[str] = frozenset(
    ...
    by for with about against between into through during before after above below to
    ...
    so than too very s t can will just don should now d ll m o re ve y ain aren
...
def get_stopwords() -> set[str]:
    return set(ENGLISH_STOPWORDS) - _PRESERVE
